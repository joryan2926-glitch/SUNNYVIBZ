...
const levelLabels: Record<string, string> = { membre_actif: "Membre actif", membre: "Membre", actif: "Membre actif", participant: "Participant", createur: "Créateur", ambassadeur: "Ambassadeur" };
const icons = ["✦", "◌", "◇", "✧"];
...
export function RewardsDashboard() {
  ...
  const loadRewards = useCallback(async (userId: string) => {
    ...
    if (!currentAccount) {
      const { error } = await client.from("reward_accounts").insert({ user_id: userId, points: 0, level: "membre" });
  ...
  const level = levelLabels[account?.level ?? "membre"] ?? "Membre actif";
